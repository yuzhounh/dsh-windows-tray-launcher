...
它会在后台执行官方命令 `npx --yes @deepseek-ai/dsh web`，从 DSH 实际输出中提取 Web UI 地址，在就绪后自动打开浏览器，并在系统托盘提供日常管理菜单。
...
- 菜单顶部有一行不可点击的状态说明：`Starting…`、`Running · 127.0.0.1:3080`、`Stopped`，或启动失败的原因，因此 **Open DSH** 灰化时原因一目了然。托盘悬停提示显示同样的内容。
